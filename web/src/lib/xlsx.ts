// Минимальный читатель xlsx: строки, значения ячеек и гиперссылки. Полноценный парсер (форматы,
// формулы, даты) не нужен — таблицы мы только читаем.
//
// Почему xlsx, а не CSV: ссылки на профили в таблицах лиги — это гиперссылки ячеек, в CSV-экспорт
// они не попадают вообще.
//
// Чистый модуль: на входе байты, никакого диска и сети — поэтому его одинаково зовут и разовые
// скрипты, и страница импорта в админке (файл там приходит из формы, а не с диска). Загрузка из
// файла/гугл-таблицы осталась в scripts/xlsx.ts, где ей и место.
